...
public static class MoreStoneScript {
    ...
    private static void RegNewCommands(Machine machine, AStonescriptGameModel gameModel) {
        ...
        machine.RegisterVariable("foe.wakeupTics", delegate {
            Enemy targetEnemy3 = GameStates.Singleton.hero.GetComponent<HeroAI>().targetEnemy;
            return (targetEnemy3 != null) ? new int?(targetEnemy3.wakeupTics) : null;
        });
        machine.RegisterVariable("foe.IsAwake", delegate {
            Enemy targetEnemy4 = GameStates.Singleton.hero.GetComponent<HeroAI>().targetEnemy;
    ...
    public static object RestartQuest(List<object> parameters, InvocationContext ctx) {
        ...
        if (gameState.GetTotalTime() >= 120) {
            Data.Quest quest = gameState.parentQuest ?? gameState.level.QuestData;
    ...
    public static object TreasureOpen(List<object> parameters, InvocationContext ctx) {
        ...
        if (treasures.Count > 0) {
            for (int i = 0; i < treasures.Count; i++) {
                TreasureItem treasure = treasures[i];
